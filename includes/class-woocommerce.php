...
class WooCommerce {
    ...
    private function __construct() {
        ...
        add_filter( 'woocommerce_product_single_add_to_cart_text', [ $this, 'add_to_cart_text_with_price' ], 10, 2 );

        // Add category to shop loop products
        add_action( 'woocommerce_shop_loop_item_title', [ $this, 'add_category_to_loop' ], 5 );
    ...
    /**
     * Add price to add to cart button text.
     */
    public function add_to_cart_text_with_price( string $text, $product ): string {
        if ( ! $product ) {
            return $text;
        }
        ...
        if ( $price ) {
            ...
            return 'ADD TO CART — ' . $formatted_price;
        }
     ...
     * Add category to shop loop products.
     */
    public function add_category_to_loop(): void {
        global $product;
        if ( ! $product ) {
            return;
        }
        ...
        if ( $categories && ! is_wp_error( $categories ) ) {
            ...
            if ( $category->slug !== 'uncategorized' ) {
                echo '<span class="gw-product-category">' . esc_html( $category->name ) . '</span>';
            }
        }
    }
